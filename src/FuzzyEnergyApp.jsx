...
const gaussmf = (x, sigma, c) => Math.exp(-((x - c) ** 2) / (2 * sigma ** 2));
...
function evalFIS(solar, wind, battery, demand) {
  ...
  const centroid =
    sumW < 1e-6 ? 50 : xs.reduce((a, x, i) => a + x * aggr[i], 0) / sumW;
    ...
    <svg width={W} height={H} style={{ display: "block" }}>
      ...
      {labels &&
        [0, 1, 2].map((i) => {
          ...
          let anchor = "middle";
          if (i === 0) anchor = "start"; // left align
          if (i === 2) anchor = "end"; // right align
          return (
            <text
              key={i}
              x={toX(xv)}
...
function GaugeChart({ value }) {
  ...
  return (
    <svg width={W} height={H}>
      ...
      <text x={cx} y={cy - 8} textAnchor="middle" fontSize="16" fill="#ffffff">
        % Renewable
      </text>

      {/* Min/Max labels */}
      <text x={25} y={H - 4} fontSize="16" fill="#ffffff">
        0%
      </text>
      <text x={W - 40} y={H - 4} fontSize="16" fill="#ffffff">
        100%
      </text>
    </svg>
  );
  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        ...
        <p style={{ fontSize: 16, color: "#9aa8bb" }}>
          Type-1 Mamdani Fuzzy Inference System{" "}
        </p>
      </div>
      ...
      {tab === 0 && (
        ...
        >
          ...
          <div
            style={{
              height: window.innerWidth < 600 ? 250 : 340,
              background: "#1e293b",
              borderRadius: 12,
              padding: 16,
              border: "1px solid #334155",
          ...
          >
            <h3 style={{ fontSize: 20, color: "#94a3b8", marginBottom: 14 }}>
              Sensor Inputs
            </h3>
            <Slider
          ...
          >
            <h3 style={{ fontSize: 20, color: "#94a3b8", marginBottom: 12 }}>
              FIS Output
            </h3>
            ...
            >
              ...
              >
                {["Low", "Medium", "High"].map((n, i) => {
                  ...
                  return (
                    <div key={n} style={{ fontSize: 13, color: MF_COLORS[i] }}>
                      {n}: <b>{mu.toFixed(3)}</b>
                    </div>
                  );
                })}
          ...
          >
            ...
            >
              {outputMFs.map((m, i) => (
                <div
                  key={m.name}
                  style={{ fontSize: 13, color: OUT_COLORS[i] }}
                >
      ...
      {tab === 2 && result && (
        ...
        >
          <h3 style={{ fontSize: 15, color: "#94a3b8", marginBottom: 12 }}>
            Aggregated Output Distribution
          </h3>
          <svg width="100%" viewBox="0 0 500 140" style={{ display: "block" }}>
            <defs>
            ...
            <polyline
              points={result.aggr
                .map(
                  (y, i) =>
                    `${10 + i * (490 / result.aggr.length)},${110 - y * 100}`,
                )
                .join(" ")}
              fill="url(#ag)"
              stroke="none"
            ...
            {/* Centroid line */}
            <line
              x1={10 + result.centroid * 4.8}
              y1={20}
              x2={10 + result.centroid * 4.8}
              y2={120}
              stroke="#fff"
              strokeWidth="2"
              strokeDasharray="4,3"
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {RULES.map((r, idx) => {
              ...
              return (
                ...
                >
                  <span
                    style={{ color: "#64748b", minWidth: 28, fontSize: 13 }}
                  >
                    R{idx + 1}
                  </span>
                  <span style={{ flex: 1, color: "#94a3b8" }}>
                    ...
                    Dem:
                    <b style={{ color: "#ef4444" }}>{labels[3][r[3] - 1]}</b>
                    {" → "}
                    <b style={{ color: col }}>{labels[4][r[4] - 1]}</b>
                  </span>
                  {fired && (
                    <span
                      style={{
      ...
      >
        Renewable Energy Grid Load Balancing Fuzzy Inference System — Nipun
        Kavinda
      </div>
    </div>
  );
